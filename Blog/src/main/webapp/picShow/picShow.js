(function($) {
	...
	jQuery.fn.actizPicShow=function(option){
		...
		option.rowSize=option.rowSize || 3;//每行显示多少个
		...
		function narrowMorePicShow(){//多图片 小图片显示
			...
			for(var i=0;i<option.data.length;i++){
				...
				morepicArray.push('<img id="'+option.data[i].id+'" height="100px" width="100px" class="bigcursor" curLoc="'+i+'" ');
		...
		function narrowOnePicShow(){//单个图片 小图片显示
			...
			onepicArray.push('<img id="'+option.data[0].id+'" height="'+option.height*3+'px" width="'+option.width*3+'px" class="bigcursor" curLoc="0" ');
